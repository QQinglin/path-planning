...
inline void JPSPathFinder::JPSGetSucc(GridNodePtr currentPtr, vector<GridNodePtr> & neighborPtrSets, vector<double> & edgeCostSets)
{
    ...
    edgeCostSets.clear();
    // currentPtr->dir(0) represents the direction vector; norm1 represents the Manhattan norm
    // |x1 - x2| + |y1 - y2| + |z1 - z2|
    const int norm1 = abs(currentPtr->dir(0)) + abs(currentPtr->dir(1)) + abs(currentPtr->dir(2));

    int num_neib  = jn3d->nsz[norm1][0]; // nsz[norm1][0] refers to the number of neighbors
    int num_fneib = jn3d->nsz[norm1][1]; // nsz[norm1][1] refers to the number of forced neighbors
    // Calculate the Index for the Direction Vector
    // transforms a 3D direction vector into a unique integer identifier by adjusting the components and weighting them with powers of 3.
    ...
    for( int dev = 0; dev < num_neib + num_fneib; ++dev) {
        Vector3i neighborIdx; // Define Neighbor Vector
        Vector3i expandDir; // Define Direction Vector
        ...
        if( dev < num_neib ) {
            ...
            // whether there is a valid jump point in this direction
        ...
        // Check for Forced Neighbors
            ...
            if( isOccupied(nx, ny, nz) ) {
                ...
                // if the cell is occupied then call jump function to check for a valid jump point
                if( !jump(currentPtr->index, expandDir, neighborIdx) ) 
                    continue;
            }
            else
...
bool JPSPathFinder::jump(const Vector3i & curIdx, const Vector3i & expDir, Vector3i & neiIdx)
{
    ...
    if( !isFree(neiIdx) ) // check for occupation
        return false;

    if( neiIdx == goalIdx ) // check for the Goal cell
        return true;

    if( hasForced(neiIdx, expDir) ) // check for Forced Neighbors
        return true;

    const int id = (expDir(0) + 1) + 3 * (expDir(1) + 1) + 9 * (expDir(2) + 1);
    const int norm1 = abs(expDir(0)) + abs(expDir(1)) + abs(expDir(2));
    int num_neib = jn3d->nsz[norm1][0]; // Number of neighbors in the given direction

    // Recursive Jump in Neighboring Directions
    // This loop iterates ove all possible neighboring directions
    for( int k = 0; k < num_neib - 1; ++k ){
        Vector3i newNeiIdx;
        // A new direction vector for each possible neighbor
        Vector3i newDir(jn3d->ns[id][0][k], jn3d->ns[id][1][k], jn3d->ns[id][2][k]);
        // Recursively calls jump to continue the search from neiIdx in the new direction newDir
            ...
            */
            if(neighborPtr -> id != 1){ //discover a new node
                ...
                */
                ...
                neighborPtr->id = 1;
                openSet.insert(make_pair(neighborPtr->gScore,neighborPtr));
                continue;
            }
                ...
                */
                ...
                // update the node's gSore, fScore and cameFrom
